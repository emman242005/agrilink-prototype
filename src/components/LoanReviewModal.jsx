      ...
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-5">
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-sm mb-6">
            ...
            <InfoRow
              label="Mobile money"
              value={loan.profiles?.mobile_money_number ? `${loan.profiles.mobile_money_provider} ${loan.profiles.mobile_money_number} — ${loan.profiles.mobile_money_holder_name || "no name"}` : "Not set"}
            />
            ...
            <div className="pt-4 border-t border-forest/10">
              <div className="flex items-end gap-4 mb-4">
                <label className="block">
                  ...
                  <input type="number" value={rate} onChange={(e) => setRate(e.target.value)} className="w-24 border border-forest/20 rounded-lg px-3 py-1.5 text-sm" />
                </label>
                <label className="block">
                  <span className="text-xs font-medium text-ink/70 mb-1 block">Term (months)</span>
                  <input type="number" value={term} onChange={(e) => setTerm(e.target.value)} className="w-24 border border-forest/20 rounded-lg px-3 py-1.5 text-sm" />
                </label>
              </div>
              <div className="flex gap-3">
                <button onClick={() => { onApprove(loan, Number(rate), Number(term)); onClose(); }} className="flex-1 bg-forest text-paper font-medium py-2.5 rounded-lg hover:bg-forestdark transition">
                  Approve & generate schedule
                </button>
                <button onClick={() => { onDecline(loan); onClose(); }} className="flex-1 border border-forest/20 text-forest/70 font-medium py-2.5 rounded-lg hover:bg-forest/5 transition">
                  Decline
                </button>
              </div>
            </div>
